...
export function ProductoModals({
  modalNueva,
  modalEditar,
  modalEliminar,
  nombre,
  ...
  const estadosProducto = getOptions('estado_producto');

  return (
    <>
      <Dialog open={modalNueva} onClose={handleCerrarNueva} PaperProps={{ sx: modalPaperSx }}>
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', pb: 1 }}>
          <Typography variant="h6" fontWeight={600}>Nuevo producto</Typography>
      ...
      <Dialog open={modalEditar} onClose={handleCerrarEditar} PaperProps={{ sx: modalPaperSx }}>
        ...
        <DialogContent>
          ...
          <Stack spacing={2}>
            ...
            />
            <FormControl size="small" fullWidth sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }}>
              <InputLabel id="editar-producto-estado-label">Estado producto</InputLabel>
              <Select
                labelId="editar-producto-estado-label"
                value={estadoProducto}
                label="Estado producto"
                onChange={(e) => setEstadoProducto(e.target.value)}
              >
